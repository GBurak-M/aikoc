...
export function initSidebarAuth() {
  ...
  /* Google oturumu kenar çubuğu üye panelini değiştirmesin */
  window.addEventListener('aikoc:google-auth', onSessionChange);
  ...
  qs('register-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      setMsg('register-msg', res.message || 'Onay e-postası gönderildi.', false);
    } catch (err) {
...
export function destroySidebarAuth() {
  ...
  window.removeEventListener('aikoc:google-auth', onSessionChange);
  unsub?.();
  unsub = null;
}
